...
if [ "${RELEASE_CHECK_ORIGIN:-yes}" = "yes" ]; then
  ...
  if [ "${git_origin_url}" != "git@github.com:enactic/dora-openarm-observer.git" ]; then
    echo "This script must be ran with working copy of enactic/dora-openarm-observer."
...
if [ "${RELEASE_TAG:-yes}" = "yes" ]; then
  echo "Tag"
  git tag -a -m "dora-openarm-observer ${version}" "${version}"
